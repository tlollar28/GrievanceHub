...
LEGACY_CROSS_CRAFT_PATHS = [
    "/upload-clock-rings",
    "/upload-runtime-report",
    "/generate-report",
    "/download-report",
    "/test-mail-handler-parser",
]
...
def test_legacy_cross_craft_routes_removed():
    paths = _openapi_paths()
    for path in LEGACY_CROSS_CRAFT_PATHS:
        assert path not in paths
        response = client.get(path) if path.startswith("/download") or path.startswith("/test") else client.post(path)
...
def test_manifest_local_paths_are_portable():
    ...
    for source_id, source in manifest["sources"].items():
        ...
        assert "cross-craft-analyzer" not in local_path.lower()
